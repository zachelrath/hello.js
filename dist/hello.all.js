(function (window, document, undefined) {
    ...
    var hello = function (append, args, clone, dataToJSON, diff, Event, extend, globalEvent, hasBinary, isEmpty, jsonp, merge, objectCreate, param, post, qs, realPath, store, unique, xhr) {
            //
            // Setup
                ...
                login: function () {
                    ...
                    if (opts.display === 'none') {
                        ...
                        var popup = window.open(//
                            ...
                            p.qs.redirect_uri + '#oauth_redirect=' + encodeURIComponent(encodeURIComponent(url)), 'Authentication', 'resizeable=true,height=' + windowHeight + ',width=' + windowWidth + ',left=' + left + ',top=' + top);
            ...
            window.hello = hello;
            return hello;
        }(utils_append, utils_args, utils_clone, utils_dataToJSON, utils_diff, utils_event, utils_extend, utils_globalEvent, utils_hasBinary, utils_isEmpty, utils_jsonp, utils_merge, utils_objectCreate, utils_param, utils_post, utils_qs, utils_realPath, utils_store, utils_unique, utils_xhr);
    var modules_dropbox = function (hello, param) {
            ...
            function format_file(o) {
                if (typeof o !== 'object' || 'Blob' in window && o instanceof Blob || 'ArrayBuffer' in window && o instanceof ArrayBuffer) {
                ...
                if (o.thumb_exists) {
                    o.thumbnail = hello.settings.oauth_proxy + '?path=' + encodeURIComponent('https://api-content.dropbox.com/1/thumbnails/' + path + '?format=jpeg&size=m') + '&access_token=' + hello.getAuthResponse('dropbox').access_token;
                }
                ...
                if (o.is_dir) {
                    ...
                    o.downloadLink = hello.settings.oauth_proxy + '?path=' + encodeURIComponent('https://api-content.dropbox.com/1/files/' + path) + '&access_token=' + hello.getAuthResponse('dropbox').access_token;
                ...
                'dropbox': {
                    ...
                    post: {
                        'me/files': function (p, callback) {
                            var path = p.data.parent, file_name = p.data.name;
                        ...
                        'me/folders': function (p, callback) {
                            ...
                            p.data = {};
                            callback('fileops/create_folder?root=@{root|sandbox}&' + param({ path: name }));
                        }
                    },
                    // Map DELETE requests 
                    del: {
    ...
    var modules_flickr = function (hello) {
            ...
            function getBuddyIcon(profile, size) {
                ...
                if (profile.nsid && profile.iconserver && profile.iconfarm) {
                    url = 'http://farm' + profile.iconfarm + '.staticflickr.com/' + profile.iconserver + '/' + 'buddyicons/' + profile.nsid + (size ? '_' + size : '') + '.jpg';
                }
                return url;
            }
    ...
    var modules_foursquare = function (hello) {
            function formatError(o) {
    ...
    var modules_github = function (hello) {
    ...
    var modules_google = function (hello, append, domInstance, hasBinary, param, _qs, xhr) {
            ...
            function formatFriends(o) {
                ...
                if ('feed' in o && 'entry' in o.feed) {
                    ...
                    for (var i = 0; i < o.feed.entry.length; i++) {
                        var a = o.feed.entry[i], pic = a.link && a.link.length > 0 ? a.link[0].href + '?access_token=' + token : null;
                        r.push({
                            id: a.id.$t,
                            name: a.title.$t,
            ...
            function paging(res) {
                ...
                if ('feed' in res && res.feed['openSearch$itemsPerPage']) {
                    var limit = int(res.feed['openSearch$itemsPerPage']['$t']), start = int(res.feed['openSearch$startIndex']['$t']), total = int(res.feed['openSearch$totalResults']['$t']);
            ...
            function Multipart() {
                // Internal body
                var body = [], boundary = (Math.random() * 10000000000).toString(32), counter = 0, line_break = '\r\n', delim = line_break + '--' + boundary, ready = function () {
            ...
            function xd(method, url, headers, body, callback) {
                ...
                if (!xd_iframe) {
                    // ID
                    xd_id = String(parseInt(Math.random() * 100000000, 10));
                    // Create the proxy window
                    xd_iframe = append('iframe', {
                ...
                var action = function () {
                    var nav = window.navigator, position = ++xd_counter, qs = param(url.match(/\?.+/)[0]);
                    var token = qs.access_token;
                    delete qs.access_token;
                ...
                google: {
                    ...
                    api: function (url, p, qs, callback) {
                        ...
                        parts.onready(function (body, boundary) {
                            ...
                            if (p.cors_support) {
                                // Deliver via 
                                xhr(p.method, _qs(url, qs), { 'content-type': 'multipart/related; boundary="' + boundary + '"' }, body, callback);
                            } else {
                                // Otherwise lets POST the data the good old fashioned way postMessage
                                xd(p.method, _qs(url, qs), { 'content-type': 'multipart/related; boundary="' + boundary + '"' }, body, callback);
                            }
                        });
                        return true;
                    }
        ...
        }(hello, utils_append, utils_domInstance, utils_hasBinary, utils_param, utils_qs, utils_xhr);
    var modules_instagram = function (hello) {
            function formatError(o) {
    ...
    var modules_linkedin = function (hello) {
            function formatError(o) {
                if (o && 'errorCode' in o) {
                    o.error = {
                        code: o.status,
    ...
    var modules_soundcloud = function (hello) {
            function formatUser(o) {
                if (o.id) {
    ...
    var modules_twitter = function (hello) {
            function formatUser(o) {
                if (o.id) {
                    if (o.name) {
                        var m = o.name.split(' ');
    ...
    var modules_windows = function (hello, hasBinary) {
            function formatUser(o) {
                if (o.id) {
    ...
    var modules_yahoo = function (hello) {
            function formatError(o) {
                if (o && 'meta' in o && 'error_type' in o.meta) {
                    o.error = {
                        code: o.meta.error_type,
            ...
            function paging(res) {
                ...
                if (res.query && res.query.count) {
                    res['paging'] = { next: '?start=' + res.query.count };
                }
            }
            var yql = function (q) {
